...
import { ClientOptions } from 'fortnox-mcp-wrapper';
...
export { ClientOptions } from 'fortnox-mcp-wrapper';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ??
      'https://api.stainless.com/api/ai/instructions/fortnox-mcp-wrapper',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the fortnox-mcp-wrapper MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new FortnoxMcpWrapper({
    logger,
