...
export const tool: Tool = {
  name: 'update_number_3_supplierinvoiceaccruals',
  description:
  ...
  inputSchema: {
    ...
    required: ['SupplierInvoiceNumber', 'SupplierInvoiceAccrual'],
    $defs: {
      fortnox_supplier_invoice_accrual: {
        type: 'object',
        properties: {
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.number3.supplierinvoiceaccruals.update(SupplierInvoiceNumber, body),
      ),
    );
  } catch (error) {
