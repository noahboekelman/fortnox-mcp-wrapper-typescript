...
export const tool: Tool = {
  name: 'update_number_3_articleurlconnections',
  description:
  ...
  inputSchema: {
    ...
    required: ['id', 'ArticleUrlConnection'],
    $defs: {
      fortnox_article_url_connection: {
        type: 'object',
        properties: {
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.articleurlconnections.update(id, body)),
    );
  } catch (error) {
