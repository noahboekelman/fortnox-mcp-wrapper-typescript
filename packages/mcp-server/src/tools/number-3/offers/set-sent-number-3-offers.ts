...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/offers/{DocumentNumber}/externalprint',
  operationId: 'externalprint',
};

export const tool: Tool = {
  name: 'set_sent_number_3_offers',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.offers.setSent(DocumentNumber)),
    );
  } catch (error) {
