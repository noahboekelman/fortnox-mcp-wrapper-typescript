...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/noxfinansinvoices/{Number}/pause',
  operationId: 'pause',
};

export const tool: Tool = {
  name: 'pause_number_3_noxfinansinvoices',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.noxfinansinvoices.pause(Number, body)),
    );
  } catch (error) {
