...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/noxfinansinvoices/{Number}/unpause',
  operationId: 'unpause',
};

export const tool: Tool = {
  name: 'unpause_number_3_noxfinansinvoices',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.noxfinansinvoices.unpause(Number)),
    );
  } catch (error) {
