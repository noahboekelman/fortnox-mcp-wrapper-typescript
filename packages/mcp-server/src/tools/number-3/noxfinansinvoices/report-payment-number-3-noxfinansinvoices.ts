...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/noxfinansinvoices/{Number}/report-payment',
  operationId: 'reportPayment',
};

export const tool: Tool = {
  name: 'report_payment_number_3_noxfinansinvoices',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.noxfinansinvoices.reportPayment(Number, body)),
    );
  } catch (error) {
