...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/scheduletimes/{EmployeeId}/{Date}/resetday',
  operationId: 'reset',
};

export const tool: Tool = {
  name: 'reset_day_number_3_scheduletimes',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.scheduletimes.resetDay(Date, body)),
    );
  } catch (error) {
