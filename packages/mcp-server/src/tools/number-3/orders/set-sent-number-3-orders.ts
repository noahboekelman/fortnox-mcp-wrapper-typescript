...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/3/orders/{DocumentNumber}/externalprint',
  operationId: 'externalprint_2',
};

export const tool: Tool = {
  name: 'set_sent_number_3_orders',
  description:
...
export const handler = async (client: FortnoxMcpWrapper, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.number3.orders.setSent(DocumentNumber)),
    );
  } catch (error) {
